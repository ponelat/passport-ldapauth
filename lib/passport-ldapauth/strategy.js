...
/**
* Transform an error into a failure, if applicable.
*
* @private
* @param {Error} err - The error
* @param {Object} options - The options object provided in .authenticate
* @returns {Object} - The message using in the failure
*/
var softErrorTransformer = function(err, options) {
  // If there is a custom transformer, use that
  // If it doesn't produce any return obj, then continue...
  var customTransformer = options.softErrorTransformer || this.options.softErrorTransformer;
  if (typeof customTransformer === 'function') {
    var failObj = customTransformer.call(this, err, options);
    if (failObj) {
      return failObj;
    }
  }
  ...
  if (err.name === 'ConstraintViolationError'){
    return { message: options.constraintViolation || 'Exceeded password retry limit, account locked' };
  }
...
var handleAuthentication = function(req, options) {
  ...
  ldap.authenticate(username, password, function(err, user) {
    ...
    if (err) {
      // Change errors into failures, if applicable
      var failObj = softErrorTransformer.call(this, err, options);

      if (failObj) {
        this.fail({ message: failObj.message }, failObj.httpCode || 401);
      }
